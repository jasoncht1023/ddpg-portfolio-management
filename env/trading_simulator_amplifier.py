...
class TradingSimulatorAmplifier:
    def __init__(self, principal, assets, start_date, end_date, rebalance_window, tx_fee_per_share):
        ...
        for stock in assets:
            df[(stock, "RSI")] = df[(stock, "Returns")].rolling(3).apply(self.__RSI)
    ...
    def restart(self):
        ...
        initial_input = np.concatenate((dev, curr_obv, prev_obv, diff, curr_rsi, prev_rsi, holdings, tangent_portfolio))
    ...
    def step(self, action):
        ...
        new_state = np.concatenate((dev, curr_obv, prev_obv, diff, curr_rsi, prev_rsi, holdings, tangent_portfolio))  # Concatenate the new state variables
