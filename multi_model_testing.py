...
if (testing_mode["ddpg_fc"] == 1):
    ...
    while not done:
        action = agent1.choose_action(observation, False)
...
if (testing_mode["ddpg_lstm"] == 1):
    ...
    while not done:
        action = agent2.choose_action(observation, False)
        print(action)
        new_state, reward, done = env.step(action)
        total_return += reward
        observation = new_state
...
if (testing_mode["ddpg_lstm_shorter"] == 1):
    ...
    print("--------------------DDPG LSTM Shorter Period--------------------")
    observation = env.restart()
    done = 0
    total_return = 0
...
if (testing_mode["ddpg_amplifier"] == 1):
    ...
    while not done:
        action = agent3.choose_action(observation, False)
        new_state, reward, done = env.step(action)
        total_return += reward
        observation = new_state
